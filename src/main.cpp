...
#if defined(NDEBUG)
# error "Hungercoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Hungercoin Signed Message:\n";
...
{
    // Hungercoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Hungercoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Hungercoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Hungercoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Hungercoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Hungercoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0486a944ec65b754b0c9171b7f389db5f5cc93638a09c30a757d044fdb0395295c8d226d5ace0349c4a5da5f369ecd49b2acc1de6e7c2367109f91bc10ee3cc487") << OP_CHECKSIG;
        CBlock block;
...
unsigned char pchMessageStart[4] = { 0xf4, 0xc4, 0xb4, 0xd4 }; // Hungercoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("HungercoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("HungercoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static HungercoinMiner(CWallet *pwallet)
{
    printf("HungercoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("hungercoin-miner");
    ...
    try { loop {
        ...
        printf("Running HungercoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("HungercoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&HungercoinMiner, pwallet));
}
